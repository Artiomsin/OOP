class Triangle: Shape {
    ...
    func draw(on canvas: inout Canvas) {
        drawLine(x1, y1, x2, y2, on: &canvas)
        drawLine(x2, y2, x3, y3, on: &canvas)
        drawLine(x3, y3, x1, y1, on: &canvas)
    }
    
    func erase(on canvas: inout Canvas) {
        eraseLine(x1, y1, x2, y2, on: &canvas)
        eraseLine(x2, y2, x3, y3, on: &canvas)
        eraseLine(x3, y3, x1, y1, on: &canvas)
                    ...
                    if i + 1 < intersections.count {
                        for x in intersections[i]...intersections[i + 1] {
                            if x >= 0, x < canvas.width, y >= 0, y < canvas.height {
                                canvas.pixels[canvas.height - 1 - y][x] = " "  // Убираем заливку
                            }
                        }
                    }
                }
    ...
    private func drawLine(_ x1: Int, _ y1: Int, _ x2: Int, _ y2: Int, on canvas: inout Canvas) {
        ...
        while true {
            if x >= 0, x < canvas.width, y >= 0, y < canvas.height {
                canvas.pixels[canvas.height - 1 - y][x] = drawSymbol
            }
            if x == x2 && y == y2 { break }
            let e2 = err * 2
    ...
    private func eraseLine(_ x1: Int, _ y1: Int, _ x2: Int, _ y2: Int, on canvas: inout Canvas) {
        ...
        while true {
            if x >= 0, x < canvas.width, y >= 0, y < canvas.height {
                canvas.pixels[canvas.height - 1 - y][x] = " "  // Erase by setting to an empty space
            }
            if x == x2 && y == y2 { break }
            let e2 = err * 2
            if e2 > -dy { err -= dy; x += sx }
